/*
 * fig2_ppnn_he3n_multibin_quantitative_final.C
 *
 * Purpose
 * -------
 * Quantitative Fig. 2 study using ppnn together with the available 3He+n
 * breakup templates in multiple energy bins.  Breakup normalizations are
 * determined before the DeltaPhi cut in the negative tail and propagated to
 * the 8/10/12 degree selections; coherent MC is normalized separately to the
 * data core.
 *
 * This multibin implementation supersedes the earlier supplied
 * fig2_ppnn_he3n_quantitative_final.C for repository purposes; the earlier
 * file is retained only in the raw archive.
 *
 ...
 * Usage
 * -----
 *   root -l -b -q 'fig2_ppnn_he3n_multibin_quantitative_final.C()'
 *
...
// Used for E0-E2, where only the ppnn template is available.
                              ...
                              double tailMax)
{
    ...
    for (int b = b1; b <= b2; ++b) {
        ...
        const double c = coherentBeforeScaled->GetBinContent(b);
        const double p = ppnnBeforeRaw->GetBinContent(b);
        const double h = he3nBeforeRaw->GetBinContent(b);
        const double ed = dataBefore->GetBinError(b);
        const double ec = coherentBeforeScaled->GetBinError(b);
        const double variance = ed*ed + ec*ec;
        if (variance <= 0.0) continue;

        const double diff = (d-c) - ppnnScale*p - he3nScale*h;
        sse += diff*diff/variance;
    }
    return sse;
}

// Simultaneous non-negative weighted least-squares fit of ppnn and He3+n.
...
static TwoComponentFit FitPPNNHe3nBefore(TH1D* dataBefore,
                                         TH1D* coherentBeforeScaled,
                                         TH1D* ppnnBeforeRaw,
                                         TH1D* he3nBeforeRaw,
                                         double tailMin,
                                         double tailMax)
{
    ...
    if (!dataBefore || !coherentBeforeScaled ||
        !ppnnBeforeRaw || !he3nBeforeRaw) return result;
...
void fig2_ppnn_he3n_multibin_quantitative_final(
...
{
    using namespace Fig2BreakupFinal;
    std::cout << "RUNNING ppnn + He3n multibin quantitative macro" << std::endl;
    gStyle->SetOptStat(0);
    ...
    if (!fHe3n || fHe3n->IsZombie()) {
        std::cerr << "Cannot open " << he3nFile << std::endl;
        return;
    }

    TFile* fOut = TFile::Open("fig2_ppnn_he3n_multibin_quantitative.root", "RECREATE");
    if (!fOut || fOut->IsZombie()) {
        std::cerr << "Cannot create output ROOT file" << std::endl;
        return;
    }

    std::ofstream txt("fig2_ppnn_he3n_multibin_quantitative.txt");
    txt << "# ppnn and He3+n normalizations fitted ONCE before the DeltaPhi cut in ["
        << tailMin << "," << tailMax << "] MeV and reused after every cut\n";
    txt << "# Every energy bin with a non-empty He3+n template uses a simultaneous non-negative ppnn + He3+n fit; other bins use ppnn only\n";
    txt << "# coherent MC is independently normalized in the core ["
        << coreMin << "," << coreMax << "] MeV before and after each cut\n";
    txt << "# The He3+n input file may contain any subset of E0-E3; availability is detected automatically from non-zero template integrals\n";
    txt << "# breakup normalizations are shape-only, not absolute cross-section predictions\n";
    txt << "# cut E_low E_high ppnn_scale he3n_scale "
        << "Nppnn_before_global Nppnn_after_global ppnn_survival_global "
        << "Nhe3n_before_global Nhe3n_after_global he3n_survival_global "
    ...
    double he3nScale[nE] = {0.0,0.0,0.0,0.0};
    double nPPNNBeforeGlobal[nE] = {0.0,0.0,0.0,0.0};
    double nHe3nBeforeGlobal[nE] = {0.0,0.0,0.0,0.0};
    ...
    for (int ie = 0; ie < nE; ++ie) {
        ...
        TH1D* h0 = dynamic_cast<TH1D*>(fHe3n->Get(Form("inc_deltaE_E%d_nodphi",ie)));
        const bool useHe3n = h0 && h0->Integral(0,h0->GetNbinsX()+1) > 0.0;
        ...
        TH1D* he3n = 0;
        if (useHe3n)
            he3n = MapToReference(h0, data, Form("he3n_before_E%d",ie));

        if (!ppnn || (useHe3n && !he3n)) {
        ...
        if (useHe3n)
            nHe3nBeforeGlobal[ie] = h0->Integral(0,h0->GetNbinsX()+1);

        const double cohScale = CoreScale(data, coh, coreMin, coreMax);
        coh->Scale(cohScale);

        if (useHe3n) {
            const TwoComponentFit fit = FitPPNNHe3nBefore(
                data, coh, ppnn, he3n, tailMin, tailMax);
            ppnnScale[ie] = fit.ppnnScale;
            he3nScale[ie] = fit.he3nScale;
        } else {
            ppnnScale[ie] = FitPPNNBefore(data, coh, ppnn, tailMin, tailMax);
        }

        ppnn->Scale(ppnnScale[ie]);
        if (he3n) he3n->Scale(he3nScale[ie]);
        ...
        if (he3n) totalAll->Add(he3n);

        StyleData(data);
        StyleCoherent(coh);
        StylePPNN(ppnn);
        if (he3n) StyleHe3n(he3n);
        ...
        if (he3n) leg->AddEntry(he3n,"^{3}He+n MC (fitted before cut)","l");
        leg->AddEntry(totalAll,he3n ? "Coherent + ppnn + ^{3}He+n" : "Coherent + ppnn","l");
        leg->Draw();
        ...
        if (he3n)
            tx.DrawLatex(0.15,0.83,Form("^{3}He+n scale = %.4g",he3nScale[ie]));
    ...
    cBefore->SaveAs("fig2_ppnn_he3n_multibin_before_normalization.pdf");
    fOut->cd();
    cBefore->Write();
    ...
    cAfter->Print("fig2_ppnn_he3n_multibin_after_prediction.pdf[");
    ...
    for (int ic = 0; ic < nCuts; ++ic) {
        ...
        for (int ie = 0; ie < nE; ++ie) {
            ...
            const bool useHe3n = nHe3nBeforeGlobal[ie] > 0.0;
            TH1D* h0 = useHe3n
                ? dynamic_cast<TH1D*>(fHe3n->Get(Form("inc_deltaE_E%d_dphi%d",ie,cuts[ic])))
                : 0;

            if (!d0 || !c0 || !p0 || (useHe3n && !h0)) {
            ...
            TH1D* he3n = 0;
            if (useHe3n)
                he3n = MapToReference(h0, data, Form("he3n_after_cut%d_E%d",cuts[ic],ie));
            if (!ppnn || (useHe3n && !he3n)) continue;
            ...
            double nHe3nAfterGlobal = 0.0;
            double he3nSurvivalGlobal = 0.0;
            if (he3n) {
                nHe3nAfterGlobal = h0->Integral(0,h0->GetNbinsX()+1);
                he3nSurvivalGlobal = nHe3nBeforeGlobal[ie] > 0.0
                    ? nHe3nAfterGlobal/nHe3nBeforeGlobal[ie] : 0.0;
            }
            ...
            txt << cuts[ic] << " " << eLow[ie] << " " << eHigh[ie] << " "
                << ppnnScale[ie] << " " << he3nScale[ie] << " "
                << nPPNNBeforeGlobal[ie] << " " << nPPNNAfterGlobal << " "
                << ppnnSurvivalGlobal << " "
                << nHe3nBeforeGlobal[ie] << " " << nHe3nAfterGlobal << " "
                << he3nSurvivalGlobal << " " << cohScale << " "
            ...
            if (he3n && nHe3nAfterGlobal > 0.0) he3n->Draw("HIST SAME");
            if (he3n) totalPPNN->Draw("HIST SAME");
            totalAll->Draw("HIST SAME");
            data->Draw("E SAME");
            ...
            if (he3n) {
                leg->AddEntry(he3n,"^{3}He+n prediction (fixed scale)","l");
                leg->AddEntry(totalPPNN,"Coherent + ppnn","l");
                leg->AddEntry(totalAll,"Coherent + ppnn + ^{3}He+n","l");
            } else {
            ...
            if (he3n) {
                tx.DrawLatex(0.15,0.89,Form("raw ppnn: %.0f #rightarrow %.0f; ^{3}He+n: %.0f #rightarrow %.0f",
                                           nPPNNBeforeGlobal[ie],nPPNNAfterGlobal,
                                           nHe3nBeforeGlobal[ie],nHe3nAfterGlobal));
                tx.DrawLatex(0.15,0.84,Form("#chi^{2}/ndf: coh %.2f, +ppnn %.2f, +^{3}He+n %.2f",
                                           chi2ndfCoh,chi2ndfPPNN,chi2ndfAll));
    ...
    std::cout << "Saved:\n"
              << "  fig2_ppnn_he3n_multibin_before_normalization.pdf\n"
              << "  fig2_ppnn_he3n_multibin_after_prediction.pdf\n"
              << "  fig2_ppnn_he3n_multibin_quantitative.root\n"
              << "  fig2_ppnn_he3n_multibin_quantitative.txt\n";
}
