/*
 * fig2_ppnn_tp_E3_quantitative_final.C
 *
 * Purpose
 * -------
 * Quantitative Fig. 2 test of ppnn plus t+p at the E3 / 366-MeV point, with
 ...
 * fig2_incoherent_4bins_1M_wide.root and fig2_tp_E3_366_wide.root.
 * Default regions: core [-10,15], tail [-60,-20], comparison [-60,40] MeV.
 *
 * Usage
 * -----
 *   root -l -b -q 'fig2_ppnn_tp_E3_quantitative_final.C()'
 *
                              ...
                              double tailMax)
{
    ...
    for (int b = b1; b <= b2; ++b) {
        ...
        const double c = coherentBeforeScaled->GetBinContent(b);
        const double p = ppnnBeforeRaw->GetBinContent(b);
        const double h = tpBeforeRaw->GetBinContent(b);
        const double ed = dataBefore->GetBinError(b);
        const double ec = coherentBeforeScaled->GetBinError(b);
        const double variance = ed*ed + ec*ec;
        if (variance <= 0.0) continue;

        const double diff = (d-c) - ppnnScale*p - tpScale*h;
        sse += diff*diff/variance;
    }
    return sse;
}

// Simultaneous non-negative weighted least-squares fit of ppnn and t+p.
...
static TwoComponentFit FitPPNNTPBefore(TH1D* dataBefore,
                                         TH1D* coherentBeforeScaled,
                                         TH1D* ppnnBeforeRaw,
                                         TH1D* tpBeforeRaw,
                                         double tailMin,
                                         double tailMax)
{
    ...
    if (!dataBefore || !coherentBeforeScaled ||
        !ppnnBeforeRaw || !tpBeforeRaw) return result;
...
void fig2_ppnn_tp_E3_quantitative_final(
...
{
    using namespace Fig2BreakupTPFinal;
    std::cout << "RUNNING ppnn + t+p E3 quantitative macro" << std::endl;
    gStyle->SetOptStat(0);
    ...
    if (!fTP || fTP->IsZombie()) {
        std::cerr << "Cannot open " << tpFile << std::endl;
        return;
    }

    TFile* fOut = TFile::Open("fig2_ppnn_tp_E3_quantitative.root", "RECREATE");
    if (!fOut || fOut->IsZombie()) {
        std::cerr << "Cannot create output ROOT file" << std::endl;
        return;
    }

    std::ofstream txt("fig2_ppnn_tp_E3_quantitative.txt");
    txt << "# ppnn and t+p normalizations fitted ONCE before the DeltaPhi cut in ["
        << tailMin << "," << tailMax << "] MeV and reused after every cut\n";
    txt << "# Only E3 is expected to contain a non-empty t+p template; E0-E2 use ppnn only\n";
    txt << "# coherent MC is independently normalized in the core ["
        << coreMin << "," << coreMax << "] MeV before and after each cut\n";
    txt << "# The t+p input file is fig2_tp_E3_366_wide.root; availability is still detected from its non-zero E3 integral\n";
    txt << "# breakup normalizations are shape-only, not absolute cross-section predictions\n";
    txt << "# cut E_low E_high ppnn_scale tp_scale "
        << "Nppnn_before_global Nppnn_after_global ppnn_survival_global "
        << "Ntp_before_global Ntp_after_global tp_survival_global "
    ...
    double tpScale[nE] = {0.0,0.0,0.0,0.0};
    double nPPNNBeforeGlobal[nE] = {0.0,0.0,0.0,0.0};
    double nTPBeforeGlobal[nE] = {0.0,0.0,0.0,0.0};
    ...
    for (int ie = 0; ie < nE; ++ie) {
        ...
        TH1D* h0 = dynamic_cast<TH1D*>(fTP->Get(Form("inc_deltaE_E%d_nodphi",ie)));
        const bool useTP = h0 && h0->Integral(0,h0->GetNbinsX()+1) > 0.0;
        ...
        TH1D* tp = 0;
        if (useTP)
            tp = MapToReference(h0, data, Form("tp_before_E%d",ie));
        ...
        if (useTP)
            nTPBeforeGlobal[ie] = h0->Integral(0,h0->GetNbinsX()+1);

        const double cohScale = CoreScale(data, coh, coreMin, coreMax);
        coh->Scale(cohScale);

        if (useTP) {
            const TwoComponentFit fit = FitPPNNTPBefore(
                data, coh, ppnn, tp, tailMin, tailMax);
            ppnnScale[ie] = fit.ppnnScale;
            tpScale[ie] = fit.tpScale;
        } else {
            ppnnScale[ie] = FitPPNNBefore(data, coh, ppnn, tailMin, tailMax);
        }

        ppnn->Scale(ppnnScale[ie]);
        if (tp) tp->Scale(tpScale[ie]);
        ...
        if (tp) leg->AddEntry(tp,"t+p MC (fitted before cut)","l");
        leg->AddEntry(totalAll,tp ? "Coherent + ppnn + t+p" : "Coherent + ppnn","l");
        leg->Draw();
        ...
        if (tp)
            tx.DrawLatex(0.15,0.83,Form("t+p scale = %.4g",tpScale[ie]));
    ...
    cBefore->SaveAs("fig2_ppnn_tp_E3_before_normalization.pdf");
    fOut->cd();
    cBefore->Write();
    ...
    cAfter->Print("fig2_ppnn_tp_E3_after_prediction.pdf[");
    ...
    for (int ic = 0; ic < nCuts; ++ic) {
        ...
        for (int ie = 0; ie < nE; ++ie) {
            ...
            const bool useTP = nTPBeforeGlobal[ie] > 0.0;
            TH1D* h0 = useTP
                ? dynamic_cast<TH1D*>(fTP->Get(Form("inc_deltaE_E%d_dphi%d",ie,cuts[ic])))
                : 0;

            if (!d0 || !c0 || !p0 || (useTP && !h0)) {
            ...
            TH1D* tp = 0;
            if (useTP)
                tp = MapToReference(h0, data, Form("tp_after_cut%d_E%d",cuts[ic],ie));
            if (!ppnn || (useTP && !tp)) continue;
            ...
            double nTPAfterGlobal = 0.0;
            double tpSurvivalGlobal = 0.0;
            if (tp) {
                nTPAfterGlobal = h0->Integral(0,h0->GetNbinsX()+1);
                tpSurvivalGlobal = nTPBeforeGlobal[ie] > 0.0
                    ? nTPAfterGlobal/nTPBeforeGlobal[ie] : 0.0;
            }
            ...
            txt << cuts[ic] << " " << eLow[ie] << " " << eHigh[ie] << " "
                << ppnnScale[ie] << " " << tpScale[ie] << " "
                << nPPNNBeforeGlobal[ie] << " " << nPPNNAfterGlobal << " "
                << ppnnSurvivalGlobal << " "
                << nTPBeforeGlobal[ie] << " " << nTPAfterGlobal << " "
                << tpSurvivalGlobal << " " << cohScale << " "
            ...
            if (tp && nTPAfterGlobal > 0.0) tp->Draw("HIST SAME");
            if (tp) totalPPNN->Draw("HIST SAME");
            totalAll->Draw("HIST SAME");
            data->Draw("E SAME");
            ...
            if (tp) {
                leg->AddEntry(tp,"t+p prediction (fixed scale)","l");
                leg->AddEntry(totalPPNN,"Coherent + ppnn","l");
                leg->AddEntry(totalAll,"Coherent + ppnn + t+p","l");
            } else {
            ...
            if (tp) {
                tx.DrawLatex(0.15,0.89,Form("raw ppnn: %.0f #rightarrow %.0f; t+p: %.0f #rightarrow %.0f",
                                           nPPNNBeforeGlobal[ie],nPPNNAfterGlobal,
                                           nTPBeforeGlobal[ie],nTPAfterGlobal));
                tx.DrawLatex(0.15,0.84,Form("#chi^{2}/ndf: coh %.2f, +ppnn %.2f, +t+p %.2f",
                                           chi2ndfCoh,chi2ndfPPNN,chi2ndfAll));
    ...
    std::cout << "Saved:\n"
              << "  fig2_ppnn_tp_E3_before_normalization.pdf\n"
              << "  fig2_ppnn_tp_E3_after_prediction.pdf\n"
              << "  fig2_ppnn_tp_E3_quantitative.root\n"
              << "  fig2_ppnn_tp_E3_quantitative.txt\n";
}
